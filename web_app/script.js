...
function initApp() {
    DOM = {
        ...
        autoRatesBtn: document.getElementById('auto-rates-btn'),
        apiStatusBoard: document.getElementById('api-status-board'), // Can be null
        spendAmount: document.getElementById('spend-amount'),
        nbgRate: document.getElementById('nbg-rate'),
        directRecv: document.getElementById('direct-recv'),
        directSpent: document.getElementById('direct-spent'),
        directRate: document.getElementById('direct-rate'),
        transferFeePct: document.getElementById('transfer-fee-pct'),
        eurUsdRate: document.getElementById('eur-usd-rate'),
        usdGelRate: document.getElementById('usd-gel-rate'),
        atmFeePct: document.getElementById('atm-fee-pct'),
        atmFeeFix: document.getElementById('atm-fee-fix'),
        streetRate: document.getElementById('street-rate'),
        labelCentralBank: document.getElementById('label-central-bank'),
        labelStreetExchange: document.getElementById('label-street-exchange'),
        localCurrencySpans: document.querySelectorAll('.local-currency'),
        resultsTbody: document.getElementById('results-tbody'),
        chartContainer: document.getElementById('chart-container'),
        bannerWrapper: document.getElementById('winner-banner'),
        bannerIcon: document.getElementById('banner-icon'),
        bannerTitle: document.getElementById('banner-title'),
        bannerSubtitle: document.getElementById('banner-subtitle'),
        bannerSavings: document.getElementById('banner-savings-value')
    };
...
function fetchWithRace(targetUrl) {
    const proxies = [
    ...
    return Promise.any(proxies).then((res) => {
        if (!res.ok)
        ...
        try {
            ...
            if (nbgData && nbgData[0] && nbgData[0].currencies) {
                for (let c of nbgData[0].currencies) {
                    if (c.code === 'EUR')
                        nbgEur = c.rate / c.quantity;
                    if (c.code === 'USD')
                        nbgUsd = c.rate / c.quantity;
                }
            }
            if (nbgEur > 0) {
